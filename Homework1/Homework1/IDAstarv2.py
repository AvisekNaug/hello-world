...
def search(node,depthBound,frontier,end,expandedNodes,open_nodes,maxSize):
    explored = []
    #print depthBound
    ...
    while not frontier.isEmpty():
        ...
        if sum(current_path[:2]) > depthBound:
            if g+h<resultlist[2]:
                resultlist = ["notFound",current_path,g+h,expandedNodes,open_nodes,maxSize]
        else:
            end_node = current_path[-1]
            if end_node == end:
                resultlist = ["found",current_path,0,expandedNodes,open_nodes,maxSize]
                return resultlist
            if end_node not in explored:
                ...
                while not tempFrontier.isEmpty():
                    ...
                    frontier.push(temp)
                    open_nodes += 1
            maxSize = max(maxSize,frontier.leng()+len(explored))
    return resultlist      
...
def Puzzle_IDAstar(start,end):
    ...
    while True:
        ...
        frontier.push(node)
        result,path,least_bound,expandedNodes,open_nodes,maxSize = search(node,depthBound,frontier,end,expandedNodes,open_nodes,maxSize)
        if result == "found" :
